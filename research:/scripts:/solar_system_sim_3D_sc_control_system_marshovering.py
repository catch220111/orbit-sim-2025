...
def load_physics_data(filename="objects.json"):
    try:
        ...
        return [{'name': 'Sun', 'pos': np.zeros(3), 'vel': np.zeros(3), 'mass': M_sun},
                ...
                {'name': 'Mars', 'pos': np.array([1.52*AU_TO_METERS, 0, 0]), 'vel': np.array([0, 24000, 0]), 'mass': 6.39e23}], datetime.now()
...
def fire_spacecraft(event):
    ...
    spacecraft_list.append({
        ...
        'prev_mars_dist': 1e20 # Track distance for approach computer
    })
...
def update(frame):
    ...
    # 2. TELEMETRY & CAPTURE COMPUTER
    capture_text.set_text("") # Clear by default
    ...
    if active_ship_idx != -1 and spacecraft_list:
        ...
        # --- NEW: MARS APPROACH COMPUTER ---
        if p_mars:
            dist_mars = np.linalg.norm(ship['pos'] - p_mars['pos'])
            # Only activate if within 200 Mm
            if dist_mars < 200e6:
                # Check trend
                prev_dist = ship.get('prev_mars_dist', 1e20)
                ship['prev_mars_dist'] = dist_mars
                
                # Calculate required dV to capture circular
                # v_circ = sqrt(GM / r)
                # We need to remove speed difference
                ...
                if dist_mars > prev_dist: # Moving AWAY (Periapsis Passed)
    ...
    ax.set_xlim(-l_y * 2.2, l_y * 2.2); ax.set_ylim(-l_y, l_y); ax.set_zlim(-2e11 * current_zoom, 2e11 * current_zoom)
